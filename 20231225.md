...
FavouriteControllerからperform_asyncで呼び出されると、performの内容が非同期で実行されます。
このワーカーで遅延が発生してもコントローラーには迷惑をかけないようになっています。

なお、UnfavouriteServiceの内容は```app/services
/unfavourite_service.rb```にあります。
```ruby
...
```
FavouriteSetviceとほとんど同じような内容になっています。違いは
